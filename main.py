...
def show_table(vacancy_dictionary, table_title):
    ...
    for language, params in vacancy_dictionary.items():
        table_rows.append([language, params['vacancies_found'], params['vacancies_processed'], params['average_salary']])
...
def create_vacancy_dict(url, pages_alias, location, responce_items_alias, responce_vacancy_cnt_alias, parameters_name, salary_count_function, headers=None):
    ...
    for language in most_popular_languages:
        ...
        page = 0
        pages_number = 1
        parameters = create_response_param(parameters_name,(vacancy_text, location, page))
        ...
        vacancies_dict[language]['vacancies_found'] = page_json[responce_vacancy_cnt_alias]        
...
if __name__ == '__main__':
    ...
    vacancies_dict_hh = create_vacancy_dict(url_hh, 'pages', 1, 'items', 'found', ('text','area','page'), predict_salary_hh)
    vacancies_dict_sj = create_vacancy_dict(url_sj, 'total', 1, 'objects','total', ('keyword','town','page'), predict_salary_sj, headers)
